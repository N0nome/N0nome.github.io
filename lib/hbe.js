  ...
  async function decrypt(decryptKey, iv, hmacKey) {
    ...
    const result = await cryptoObj.subtle.decrypt({
      ...
      document.addEventListener('DOMContentLoaded', function () {
        var tocDivs = document.querySelectorAll(
...
const decryptedHtml = container.innerHTML;

// 把解密后正文放到页面上（你已有这步）
container.innerHTML = decryptedHtml;

// 重新调用 toc() 并插入到 actions_desktop.ejs 渲染的位置
// 这里假设 actions_desktop.ejs 中的 <div id="toc"> 还在页面上
const tocHtml = window.toc(decryptedHtml);
