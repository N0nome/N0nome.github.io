...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  async function decrypt(decryptKey, iv, hmacKey) {
	...
	document.addEventListener('DOMContentLoaded', function () {
  	...
  	// 针对可能的其他目录容器（例如类名为 toc 或 toc-container）
  	var tocDivs = document.querySelectorAll('#hexo-blog-encrypt .toc, #hexo-blog-encrypt #toc, #hexo-blog-encrypt .toc-container');
  	if (tocDivs && tocDivs.length > 0) {
    		for (var idx = 0; idx < tocDivs.length; idx++) {
      			tocDivs[idx].style.display = 'block';
      			tocDivs[idx].style.visibility = 'visible';
    		}
  	}
	});
